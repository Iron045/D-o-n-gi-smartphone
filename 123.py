...
def train_model(model_type):
    ...
    return model, mae, mse, r2, mean_cv_score, cols
...
model_type = st.selectbox('Chọn mô hình dự đoán', ['Linear Regression', 'Lasso Regression', 'Ridge Regression', 'Neural Network'])

# Tải và huấn luyện mô hình
model, mae, mse, r2, mean_cv_score, cols = train_model(model_type)
...
if st.button("Dự đoán giá"):
    try:
        ...
        st.write(f"**R²**: {r2:.4f}")
        st.write(f"**Mean CV Score** (Cross-validation): {mean_cv_score:.4f}")
